...
const YearlyBreakup = () => {
  ...
  const optionscolumnchart: any = {
    ...
    tooltip: {
      ...
      y: {
        formatter: (val: number) => `${val} Ectolitros`,
      },
      custom: ({ series, seriesIndex }: any) => {
        const years = ['2023', '2024']; 
        return `<div style="padding: 10px; font-size: 14px;">
                  <strong>${years[seriesIndex]}</strong><br/>
                  ${series[seriesIndex]} Ectolitros
                </div>`;
      }
    },
    stroke: {
        ...
        <Grid item xs={7} sm={7}>
          ...
          <Stack direction="row" spacing={1} mt={1} alignItems="center">
            ...
            <Typography variant="subtitle2" fontWeight="600">
              {((seriescolumnchart[1] - seriescolumnchart[0]) / seriescolumnchart[0] * 100).toFixed(1)}%
            </Typography>
